...
public class Player2 {
	...
	public Player2(int player,int depth) {
		this.player= player;
		this.depth=depth;
		cut= new int[depth+1];
	...
	public String elaborateMove() {
		...
		if(numberMove==0 && player== WHITE) {
			...
			TimerThread tt= new TimerThread(Thread.currentThread());
			tt.start();
	...
	private int negamaxCut(int depth, int alfa, int beta,int player,long start) {
	    if(System.currentTimeMillis()-start > 950){ elapsed=true; return -1;}
		Move curBest= null;
		boolean hit= false;
		Move[]moves =null;
	...
	private int negamax(int depth, int alfa, int beta,int player,long start) {
		if(System.currentTimeMillis()-start > 970){ elapsed=true; return -1;}
		Move curBest= null;
		boolean hit= false;
		int flag = UPPER_BOUND;
		int best=0;
